...
def numhart(low_diam_km, upper_diam_km):
    hartlow = (10.0 ** -1.14) * low_diam_km ** -1.83
    harthigh = (10.0 ** -1.14) * upper_diam_km ** -1.83
...
def numtrask(low_diam_km, upper_diam_km):
    trasklow = (10.0 ** -1.1) * low_diam_km ** -2.0
    traskhigh = (10.0 ** -1.1) * upper_diam_km ** -2.0
...
def makegruns(minlogmassgrams=-18.0, maxlogmassgrams=2.0):
    ...
    while masslog <= maxlogmassgrams:
        ...
        grunsize = 0.01 * (
                ((1.0 / (impactor_density / 1000.0)) * ((3.0 / 4.0) * (1.0 / pi) * massg)) ** (1.0 / 3.0))
...
def hohocratersize(impradius):
    ...
    impactor_mass = ((4.0 * pi) / 3.0) * impactor_density * (impradius ** 3.0)
...
def diffuse_to_threshold(dDmax_strategy, diam, visibility_threshold):
    ...
    current_radius = diam / 2.0
    current_range = 1.5 * current_radius

    px_r = (current_radius / sscale)
    x, y = np.ogrid[:domainsizepx, :domainsizepx]
    distance_from_center = (((x - xcen) ** 2.0 + (y - ycen) ** 2.0) ** 0.5) * sscale
    range_from_center = distance_from_center / current_radius
    incrat = np.logical_and(range_from_center >= 0.0, range_from_center < 0.98)
    rim = np.logical_and(range_from_center >= 0.98, range_from_center < 1.02)
    outcrat = np.logical_and(range_from_center >= 1.02, range_from_center < 1.5)

    sigma = 0.0  # 35

    dDmax = dDmax_strategy(diam, sigma)

    interior_floor = -(dDmax - 0.036822095) * diam

    # FRESH CRATER MODEL:  Basically what's in Fassett and Thomson, 2014, with a little fix implremented at the rim (
    # whoops) forced to have a flat floor to match dDMax at small sizes
    u[incrat] = (-0.228809953 + 0.227533882 * range_from_center[incrat] + 0.083116795 * (
            range_from_center[incrat] ** 2.0) - 0.039499407 * (range_from_center[incrat] ** 3.0)) * diam
    u[rim] = 0.036822095 * diam
    u[outcrat] = (0.188253307 - 0.187050452 * range_from_center[outcrat] + 0.01844746 * (
            range_from_center[outcrat] ** 2.0) + 0.01505647 * (range_from_center[outcrat] ** 3.0)) * diam
    u[u < interior_floor] = interior_floor
    ...
    if size < 10.0:
        ...
        if sulog > grundiameterslog[-1]:
            ...
            interpfreqs = [cratcalc2(10 ** (grundiameterslog[-1] - lstep - epsilon - 3.0),
                           ...
                           (10 ** log10(npfcalc(0.01)) - 10 ** log10(npfcalc(10 ** (-2.0 + lstep))))]
        ...
        globalfreq = surface_area * (npfcalc(size_km) - npfcalc(size_km_upper))
    else:
...
def solve_for_t(kT_this_size):
    equation = lambda t: (363.58 * (t**5)) - (2954 * (t**4)) + (8953 * (t**3)) - (13814 * (t**2)) + (16695 * t) - kT_this_size
